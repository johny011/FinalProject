...
namespace Final_project
{
    public partial class Accounting : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                int RowAnalyzes = DB.DataTable("select celendarRAid,price, Analysisname from Analyzes A,celendarRA CRA ,RequiredAnalysis RA where A.Analysisnumber = RA.Analysisnumber and RA.RequiredAnalysisid = CRA.RequiredAnalysisid and RA.Admissionid = '" + textBox1.Text + "' ").Rows.Count;
                int RowPictures= DB.DataTable("select celendarRPid,price,picturesname from pictures P,Requiredpictures RP,celendarRP CRP where P.picturesid=RP.picturesid and RP.Requiredpicturesid=CRP.Requiredpicturesid and RP.Admissionid='" + textBox1.Text + "' ").Rows.Count;
                if (RowAnalyzes!=0)
